...
def build_wrapper_script(m: RunManifest,
    ...
    # HF-download env is BASE-AWARE. Two conflicting failure modes (both learned
    # the hard way — see pod.py):
    #   - Klein/small weights: the pods' route to HF's Xet backend STALLS
    #     mid-stream -> disable Xet + force hf_transfer (Rust downloader).
    #   - FLUX.2-dev large shards (61GB transformer + 48GB Mistral TE): the
    #     regular/hf_transfer path REJECTS them ("file is too large ... install
    #     hf_xet") -> must download via Xet (hf_xet). Disabling Xet here CRASHES
    #     the dev load at startup (observed 2026-07).
    # So pick per arch: flux2 (dev) -> Xet ON / hf_transfer OFF; else -> the
    # Klein Xet-stall workaround.
    _needs_xet = False
    try:
        import yaml as _yaml
        _lcfg = os.path.join(contract.local_run_dir(run), contract.DERIVED_CONFIG_FILE)
        _arch = (_yaml.safe_load(open(_lcfg))["config"]["process"][0]["model"].get("arch") or "")
        # krea2: single 26GB raw.safetensors — plain/hf_transfer path crawled at
        # 2-10 MB/s (2026-09-07, 1h+ ETA) while Xet did ~228 MB/s on the same pod.
        # 2026-09-08: Klein too — the June-2026 Xet stall is gone; on the same pod
        # the plain/hf_transfer path crawled at 5-11 MB/s (48 min ETA for 18GB)
        # while Xet did ~221 MB/s. Non-Xet path is now the stall, not the fix.
        _needs_xet = _arch in ("flux2", "krea2", "flux2_klein_9b")
    except Exception:
        _needs_xet = False
    if _needs_xet:
        hf_env_block = (
            "# (a2/a3) LARGE-SHARD base (FLUX.2-dev): download via Xet (hf_xet).\n"
            "# The Rust/regular downloader rejects the 61GB shards ('file too\n"
            "# large, install hf_xet'); Xet is the chunked CAS path that handles\n"
            "# them. Do NOT disable Xet here.\n"
            "export HF_HUB_ENABLE_HF_TRANSFER=0\n"
            "unset HF_HUB_DISABLE_XET || true"
        )
    else:
        hf_env_block = (
            "# (a2) Force the robust Rust downloader; the plain Python one stalls\n"
            "# on dead sockets (Klein 9B hung at 10%/88%, 2026-06).\n"
            "export HF_HUB_ENABLE_HF_TRANSFER=1\n"
            "# (a3) Bypass the Xet protocol; the pods' route to it dies mid-stream\n"
            "# (HF Apr-2026 XET large-file stall). Falls back to LFS/CDN.\n"
...
# (a2/a3) HF-download env — BASE-AWARE (see build_wrapper_script). Klein/small
# weights disable Xet + force hf_transfer (Xet-stall workaround); FLUX.2-dev
# large shards keep Xet ON (the regular downloader rejects 61GB files).
{hf_env_block}
